...
publishBtn.onclick = async () => {
    ...
    try {
        ...
        showNotification('Error publishing resource', 'error');
    }
};
...
updateBtn.onclick = async () => {
    ...
    try {
        const response = await fetch(`/api/resources/${currentEditId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(resource)
        });
...
saveQuickLinkBtn.onclick = async () => {
    ...
    try {
        ...
        showNotification('Error adding quick link', 'error');
    }
};
...
async function deleteCommonLink(id) {
    if (confirm('Delete this quick link?')) {
        try {
            ...
            showNotification('Error deleting quick link', 'error');
        }
    }
}
